...
//		ss << "/storage/emulated/0/debug_color_plate_" << i << ".jpg";
//		imwrite(ss.str(), color_Plates[i].getPlateMat());
//		//color_result_Plates.push_back(color_Plates[i]);
//	}
	LOGI(
			"color_Plates.size=%d,color_result_Plates.size=%d", color_Plates.size(), color_result_Plates.size());
...
//			stringstream ss(stringstream::in | stringstream::out);
//			ss << "/storage/emulated/0/debug_sobel_plate_" << i << ".jpg";
//			imwrite(ss.str(), sobel_Plates[i].getPlateMat());
//			//color_result_Plates.push_back(color_Plates[i]);
//		}
		LOGI(
				"sobel_Plates.size=%d,sobel_result_Plates.size=%d", sobel_Plates.size(), sobel_result_Plates.size());

//		if (sobel_result_Plates.size() != sobel_Plates.size()
//				&& sobel_result_Plates.size() == 0) {
//			for (int i = 0; i < sobel_Plates.size(); ++i) {
...
int CPlateDetect::showResult(const Mat& result) {
	...
	if (nCols <= img_window.cols && nRows <= img_window.rows) {
		...
		if (result_middle.rows > img_window.rows) {
			...
			resize(result_middle, result_resize, Size(), scale, scale,
					CV_INTER_AREA);
	...
	Mat imageRoi = img_window(
			Rect((RESULTWIDTH - result_resize.cols) / 2,
					(RESULTHEIGHT - result_resize.rows) / 2, result_resize.cols,
					result_resize.rows));
