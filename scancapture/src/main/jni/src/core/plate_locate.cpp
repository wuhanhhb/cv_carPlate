...
int CPlateLocate::colorSearch(const Mat& src, const Color r, Mat& out,
	...
	if (m_debug) {
		imwrite("/storage/emulated/0/match_grey.jpg", match_grey);
	}
	...
	Mat element = getStructuringElement(MORPH_RECT,
			Size(color_morph_width, color_morph_height));
	morphologyEx(src_threshold, src_threshold, MORPH_CLOSE, element);

	if (m_debug) {
		imwrite("/storage/emulated/0/color.jpg", src_threshold);
	}
...
int CPlateLocate::sobelSecSearchPart(Mat& bound, Point2f refpoint,
	...
	if (bFindLeftRightBound(tempBoundThread, posLeft, posRight)) {
		...
		if (m_debug) {
			imwrite("/storage/emulated/0/repaireimg1.jpg", bound_threshold);
		}
		//两边的区域不要
		...
		if (m_debug) {
			imwrite("/storage/emulated/0/repaireimg2.jpg", bound_threshold);
		}
	}
...
int CPlateLocate::sobelSecSearch(Mat& bound, Point2f refpoint,
	...
	//}
	if (m_debug) {
		imwrite("/storage/emulated/0/sobelSecSearch.jpg", bound_threshold);
	}
	vector<vector<Point> > contours;
	findContours(bound_threshold, contours, // a vector of contours
			CV_RETR_EXTERNAL, // 提取外部轮廓
...
int CPlateLocate::sobelOper(const Mat& in, Mat& out, int blurSize, int morphW,
	...
	// 分割
	Mat mat_threshold;
	double otsu_thresh_val = threshold(grad, mat_threshold, 0, 255,
			CV_THRESH_OTSU + CV_THRESH_BINARY);
	// 腐蚀和膨胀
...
void DeleteNotArea(Mat& inmat) {
	...
	if (BLUE == plateType) {
		...
		Mat tmp = input_grey(
				Rect_<double>(w * 0.15, h * 0.15, w * 0.7, h * 0.7));
		int threadHoldV = ThresholdOtsu(tmp);
		...
		// CV_THRESH_BINARY);
//		if (m_debug) {
//			imwrite("/storage/emulated/0/inputgray2.jpg", img_threshold);
//		}
...
int CPlateLocate::deskew(const Mat& src, const Mat& src_b,
		vector<RotatedRect>& inRects, vector<CPlate>& outPlates) {
	Mat mat_debug;
	...
	for (size_t i = 0; i < inRects.size(); i++) {
		...
		if (roi_angle - m_angle < 0 && roi_angle + m_angle > 0) {
			...
			if ((roi_angle - 3 < 0 && roi_angle + 3 > 0) || 90.0 == roi_angle
				...
				if (!rotation(bound_mat, rotated_mat, roi_rect_size,
						roi_ref_center, roi_angle))
					continue;

				if (!rotation(bound_mat_b, rotated_mat_b, roi_rect_size,
						roi_ref_center, roi_angle))
					continue;
			...
			if (deskew_mat.cols * 1.0 / deskew_mat.rows > 2.3
				...
				if (deskew_mat.cols >= WIDTH || deskew_mat.rows >= HEIGHT)
					resize(deskew_mat, plate_mat, plate_mat.size(), 0, 0,
							INTER_AREA);
				else
					resize(deskew_mat, plate_mat, plate_mat.size(), 0, 0,
							INTER_CUBIC);
...
bool CPlateLocate::rotation(Mat& in, Mat& out, const Size rect_size,
	...
	float x =
			in_large.cols / 2 - center.x > 0 ? in_large.cols / 2 - center.x : 0;
	float y =
			in_large.rows / 2 - center.y > 0 ? in_large.rows / 2 - center.y : 0;
...
bool CPlateLocate::isdeflection(const Mat& in, const double angle,
	...
	if (maxlen - len[1] > nCols / 32 || len[1] - minlen > nCols / 32) {
		// 如果斜率为正，则底部在下，反之在上
		double slope_can_1 = double(len[2] - len[0]) / double(comp_index[1]); //��ֱ�ߵ�б��
...
bool CPlateLocate::calcSafeRect(const RotatedRect& roi_rect, const Mat& src,
	...
	float br_x =
			boudRect.x + boudRect.width < src.cols ?
					boudRect.x + boudRect.width - 1 : src.cols - 1;
	float br_y =
			boudRect.y + boudRect.height < src.rows ?
...
int CPlateLocate::sobelOperT(const Mat& in, Mat& out, int blurSize, int morphW,
	...
	if (mat_blur.channels() == 3)
		...
		mat_gray = mat_blur;
	if (m_debug) {
		imwrite("/storage/emulated/0/grayblure.jpg", mat_gray);
	}
	...
	if (m_debug) {
		imwrite("/storage/emulated/0/graygrad.jpg", grad);
	}
	...
	double otsu_thresh_val = threshold(grad, mat_threshold, 0, 255,
			CV_THRESH_OTSU + CV_THRESH_BINARY);
	if (m_debug) {
		imwrite("/storage/emulated/0/grayBINARY.jpg", mat_threshold);
	}

	Mat element = getStructuringElement(MORPH_RECT, Size(morphW, morphH));
	morphologyEx(mat_threshold, mat_threshold, MORPH_CLOSE, element);
	if (m_debug) {
		imwrite("/storage/emulated/0/phologyEx.jpg", mat_threshold);
	}
...
int CPlateLocate::plateSobelLocate(Mat src, vector<CPlate>& candPlates,
	...
	for (size_t i = 0; i < bound_rects_part.size(); i++) {
		...
		float width =
				x + bound_rect.width < src.cols ?
						bound_rect.width : src.cols - x;
		float height =
				y + bound_rect.height < src.rows ?
