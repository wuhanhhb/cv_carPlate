...
int CCharsSegment::charsSegment(Mat input, vector<Mat>& resultVec, int index) {
	...
	if (BLUE == plateType) {
		...
		Mat tmp = input_grey(Rect_<double>(w * 0.1, h * 0.1, w * 0.8, h * 0.8));
		int threadHoldV = ThresholdOtsu(tmp);
		// imwrite("/storage/emulated/0/inputgray2.jpg", input_grey);
	...
	if (m_debug) {
		...
		ss << grobal_sdcard << "/debug_char_threshold_" << index << ".jpg";
	...
	if (m_debug || 1) {
		stringstream ss(stringstream::in | stringstream::out);
		ss << grobal_sdcard << "/debug_char_clearLiuDing_" << index << ".jpg";
		imwrite(ss.str(), img_threshold);
	}
	...
	//针对光照/划痕等因素，采取了新的定位方式 136 x 36
	if(sortedRect.size() < 7 && sortedRect.size() > 4){
		//19 38 57 76 95 114 133
		sortedRect.clear();
		int w = (img_threshold.cols - 1) / 7;
		int y = 1;
		int h = img_threshold.rows - y - 1;
		int x = 1;
		LOGE("========================segment here begin");
		for (int i = 0; i < 7; i++) {
			x = 1 + i * w;
			Rect mr(x, y, w, h);
			//LOGE("========================segment here begin %d : %d %d ",i,x,y);
            if(i == 0){
            	Mat specMat(img_threshold, mr);
            	//Mat spec_contours;
               //specMat.copyTo(spec_contours);

            	vector<vector<Point> > ccps;
                	findContours(specMat, ccps, // a vector of contours
                			CV_RETR_EXTERNAL, // retrieve the external contours
                			CV_CHAIN_APPROX_NONE); // all pixels of each contours
LOGE("========================segment here ");
                	vector<vector<Point> >::iterator ccpi = ccps.begin();
                	// 将不符合特定尺寸的字符块排除出去
                	while (ccpi != ccps.end()) {
                		Rect mrr = boundingRect(Mat(*ccpi));
                		if(mrr.width > 9){
                			Mat axo(img_threshold, mrr);
                			if (verifyCharSizes(axo)){
                				sortedRect.push_back(mrr);
                			}
                		}
                		++ccpi;
                	}
            	...
            	sortedRect.push_back(mr);
            }
            if (m_debug){
				Mat specMat(img_threshold, mr);
				stringstream ss(stringstream::in | stringstream::out);
				ss << grobal_sdcard << "/segment_" << i << ".jpg";
				imwrite(ss.str(), specMat);
            }
    	}
    	LOGE("========================segment here end %d",sortedRect.size());
    }

	std::sort(sortedRect.begin(), sortedRect.end(), SortByM1);
	...
	if (m_debug || train_ann) {
		if (specIndex < sortedRect.size()) {
			Mat specMat(img_threshold, sortedRect[specIndex]);
			stringstream ss(stringstream::in | stringstream::out);
			ss << grobal_sdcard << "/debug_specMat_" << index << ".jpg";
			imwrite(ss.str(), specMat);
		}
	}
	...
	if (m_debug || train_ann) {
		...
		stringstream ss(stringstream::in | stringstream::out);
		ss << grobal_sdcard << "/debug_chineseMat_" << index << ".jpg";
	...
	for (size_t i = 0; i < newSortedRect.size(); i++) {
		...
		if (i == 0 && (m_debug || train_ann)) {
			stringstream ss(stringstream::in | stringstream::out);
			ss << grobal_sdcard << "/debug_char_auxRoi_" << index << "_" << (i)
					<< ".jpg";
