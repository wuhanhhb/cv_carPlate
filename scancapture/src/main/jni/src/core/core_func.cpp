...
const int mid = img.rows >> 1;
  int top = 0;
  int bottom = img.rows;
//  LOGE("clearLiuDingOnly begin");
  for (int i = 0; i < img.rows; i++) {
//      LOGE("clearLiuDingOnly %d %f ",i, jump.at<float>(i));
      if (jump.at<float>(i) <= x) {
        if(i < mid){
          top = i;
        }else if(bottom > i){
  ...
  LOGE("clearLiuDingOnly %d %d %d ",mid,top, bottom);
